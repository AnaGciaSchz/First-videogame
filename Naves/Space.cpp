...
void Space::updateMoveRight(Actor* dynamicAct) {
    if (dynamicAct->vx > 0) {
        ...
        //como estamos calculando para movernos a la derecha, no nos importa nuestro punto izquierdo
        ...
        for (auto const& staticAct : staticActors) {
            ...
            //miramos si colisonamos con elemento, al ir a la derecha lo haremos con su izquierda
            ...
            if ((rightDynamic + dynamicAct->vx) >= leftStatic
                ...
                if (possibleMovement >= leftStatic - rightDynamic) {//movernos más de lo que nos permite el movimiento
                    // La distancia es MENOR que nuestro movimiento posible
                    // Tenemos que actualizar el movimiento posible a uno menor
                    possibleMovement = leftStatic - rightDynamic; //nos movemos lo que podamos
                }
            }
        }
